...
test.skip('single-config', done => {
    const webpackProc = runAndGetWatchProc(__dirname, ['--entry', './index.js', '--config', './webpack.config.js', '--output-filename', '[name].js', '--output-chunk-filename', '[id].chunk.js', '--target', 'async-node', '--watch']);
