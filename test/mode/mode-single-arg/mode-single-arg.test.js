...
describe('mode flags', () => {
    ...
    it('should load a development config when --mode=development is passed', () => {
        const { stdout, exitCode } = run(__dirname, ['--mode', 'development']);
    ...
    it('should load a none config when --mode=none is passed', () => {
        const { stdout, exitCode } = run(__dirname, ['--mode', 'none']);
    ...
    it('should pick mode form NODE_ENV', () => {
        const { stdout, exitCode } = run(__dirname, [], false, [], { NODE_ENV: 'development' });
        expect(exitCode).toBe(0);
