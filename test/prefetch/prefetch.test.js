...
describe('Prefetch Flag', () => {
    ...
    it('Should load the prefetched file', () => {
        const { stdout, stderr, exitCode } = run(__dirname, ['--prefetch', './src/p.js'], false);
    ...
    it('Should err when the prefetched file is absent', () => {
        const { stdout, stderr, exitCode } = run(__dirname, ['--prefetch', './src/somefile.js'], false);
    ...
    it('Should err when flag value is not supplied', () => {
        const { stdout, stderr, exitCode } = run(__dirname, ['--prefetch'], false);
        // Should contain the error message
