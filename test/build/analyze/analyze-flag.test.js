...
describe('"analyze" option', () => {
    it("should not load webpack-bundle-analyzer plugin twice with --analyze flag and plugin", async () => {
        ...
        expect(
            normalizeStdout(stdout).match(/Webpack Bundle Analyzer saved report to/g),
        ).toHaveLength(1);
    });

    it("should not load webpack-bundle-analyzer plugin twice with -a alias and plugin", async () => {
        const { exitCode, stderr, stdout } = await run(__dirname, [
            "-c",
            "./analyze.config.js",
            "-a",
