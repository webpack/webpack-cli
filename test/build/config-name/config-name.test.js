...
describe("--config-name flag", () => {
    ...
    it("should work with multiple values for --config-name and multiple configurations", async () => {
        const { exitCode, stderr, stdout } = await run(__dirname, ["-c", "./function-config.js", "-c", "./single-other-config.js", "--config-name", "first", "--config-name", "four"], false);
    ...
    it("should log error if multiple configurations are not found #1", async () => {
        const { exitCode, stderr, stdout } = await run(__dirname, ["--config-name", "test", "--config-name", "bar", "-c", "single-config.js"], false);
    ...
    it("should log error if multiple configurations are not found #2", async () => {
        const { exitCode, stderr, stdout } = await run(__dirname, ["--config-name", "first", "--config-name", "bar", "-c", "single-config.js"], false);
    ...
    it("should work with multiple values for --config-name when the config is a function", async () => {
        const { exitCode, stderr, stdout } = await run(__dirname, ["--config", "function-config.js", "--config-name", "first", "--config-name", "third"], false);
