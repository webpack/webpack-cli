...
describe('mode flags with config', () => {
    it('should run in production mode when --mode=production is passed', async () => {
        const { exitCode, stderr, stdout } = await runAsync(__dirname, ['--mode', 'production', '--config', './webpack.config.js']);
    ...
    it('should run in development mode when --mode=development is passed', async () => {
        const { exitCode, stderr, stdout } = await runAsync(__dirname, ['--mode', 'development', '--config', './webpack.config.js']);
    ...
    it('should run in none mode when --mode=none is passed', async () => {
        const { exitCode, stderr, stdout } = await runAsync(__dirname, ['--mode', 'none', '--config', './webpack.config.js']);
    ...
    it('should use mode flag over config', async () => {
        const { exitCode, stderr, stdout } = await runAsync(__dirname, ['--mode', 'production', '-c', 'webpack.config2.js']);
    ...
    it('should use mode from flag over NODE_ENV', async () => {
        const { exitCode, stderr, stdout } = await runAsync(__dirname, ['--mode', 'none', '-c', 'webpack.config2.js'], false, [], {
            NODE_ENV: 'production',
        });
    ...
    it('should use mode from config over NODE_ENV', async () => {
        const { exitCode, stderr, stdout } = await runAsync(__dirname, ['-c', 'webpack.config2.js']);
    ...
    it('should use mode from config when multiple config are supplied', async () => {
        const { exitCode, stdout, stderr } = await runAsync(__dirname, ['-c', 'webpack.config3.js', '-c', 'webpack.config2.js']);
    ...
    it('only config where mode is absent pick up from NODE_ENV', async () => {
        const { exitCode, stderr, stdout } = await runAsync(__dirname, ['-c', './webpack.config3.js', '-c', './webpack.config2.js'], {
            env: {
                NODE_ENV: 'production',
            },
        });
