        ...
        if (flag.configs.filter((config) => config.type === 'boolean').length > 0) {
            it(`should config --${flag.name} correctly`, async () => {
                if (flag.name === 'optimization-split-chunks') {
                    ...
                    const { exitCode, stderr } = await runAsync(__dirname, [`--${flag.name}`]);
        ...
        if (
            ...
            it(`should config --${flag.name} correctly`, async () => {
                if (flag.name === 'optimization-split-chunks-chunks') {
                    const { exitCode, stderr, stdout } = await runAsync(__dirname, [`--${flag.name}`, 'initial']);

                    expect(exitCode).toBe(0);
                    expect(stderr).toBeFalsy();
                    expect(stdout).toContain(`chunks: 'initial'`);
                } else if (flag.name === 'optimization-mangle-exports') {
                    const { exitCode, stderr, stdout } = await runAsync(__dirname, ['--optimization-mangle-exports', 'size']);

                    expect(exitCode).toBe(0);
                    expect(stderr).toBeFalsy();
                    expect(stdout).toContain(`mangleExports: 'size'`);
                } else if (flag.name === 'optimization-used-exports') {
                    const { exitCode, stderr, stdout } = await runAsync(__dirname, ['--optimization-used-exports', 'global']);
                    ...
                    const { exitCode, stderr, stdout } = await runAsync(__dirname, ['--optimization-side-effects', 'flag']);

                    expect(exitCode).toBe(0);
                    expect(stderr).toBeFalsy();
                    expect(stdout).toContain(`${propName}: 'flag'`);
                } else {
                    const { exitCode, stderr, stdout } = await runAsync(__dirname, [`--${flag.name}`, 'named']);
