...
describe('entry flag', () => {
    it('should resolve the path to src/index.cjs', async () => {
        const { exitCode, stderr, stdout } = await runAsync(__dirname, ['--entry', './src/index.cjs', '-o', './dist/']);
    ...
    it('should load ./src/a.js as entry', async () => {
        const { exitCode, stderr, stdout } = await runAsync(__dirname, ['--entry', './src/a.js']);
    ...
    it('should throw error for invalid entry file', async () => {
        const { exitCode, stderr, stdout } = await runAsync(__dirname, ['--entry', './src/test.js']);
