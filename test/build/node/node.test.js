...
describe('node flags', () => {
    ...
    it('throws an error on supplying unknown flags', async () => {
        const { exitCode, stderr, stdout } = await runAsync(__dirname, [], { nodeOptions: ['--unknown'] });
    ...
    it('throws an error if no values were supplied with --max-old-space-size', async () => {
        const { exitCode, stderr, stdout } = await runAsync(__dirname, [], { nodeOptions: ['--max-old-space-size'] });
    ...
    it('throws an error if an illegal value was supplied with --max-old-space-size', async () => {
        const { exitCode, stderr, stdout } = await runAsync(__dirname, [], { nodeOptions: ['--max_old_space_size=1024a'] });
