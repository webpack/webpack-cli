...
import webpack, { Argument, Configuration } from "webpack";
import {
  Compiler,
  MultiCompiler,
...
type WebpackCommand = Command & { options: typeof Option[]; _args: typeof Option };
const isMultipleCompiler = (compiler: WebpackCompiler): compiler is MultiCompiler => {
  return (compiler as MultiCompiler).compilers as unknown as boolean;
};

const isPromise = <T>(val: Promise<T>): val is Promise<T> => {
  return typeof (val as unknown as Promise<T>).then === "function";
};
class WebpackCLI implements WebpackCliInterface {
  colors: Colors;
  logger: Logger;
  isColorSupportChanged: boolean | undefined;
  builtInOptionsCache: BuiltInOptions[] | undefined;
  webpack!: typeof webpack;
  program: WebpackCommand & { forHelp: boolean };
  constructor() {
  ...
  checkPackageExists(packageName: string) {
  ...
  async tryRequireThenImport<T>(module: Module, handleError = true): Promise<T> {
    let result;
  ...
  loadJSONFile<T = any>(pathToFile: Path, handleError = true): T {
    let result;
  ...
  makeOption(command: WebpackCommand | ExtendedCommand, option: BuiltInOptions) {
    ...
    if (option.configs) {
      ...
      const mainOptionType: MainOptionType = new Set();
    ...
    if (mainOption.type.size === 1) {
      if (mainOption.type.has(Number)) {
        ...
        const optionForCommand: typeof Option = new Option(mainOption.flags, mainOption.description)
  ...
  getBuiltInOptions(): BuiltInOptions[] {
    ...
    const builtInFlags: BuiltInFlag[] = [
      // For configs
      {
        name: "config",
        alias: "c",
    ...
    const options: BuiltInOptions[] = ([] as BuiltInFlag[])
      .concat(
        builtInFlags.filter(
      ...
      .concat(coreFlags)
      .map((option): BuiltInOptions => {
        (option as BuiltInOptions).helpLevel = minimumHelpFlags.includes(option.name)
          ? "minimum"
          : "verbose";
        return option as BuiltInOptions;
      });
  ...
  async run(args: Parameters<WebpackCommand["parseOptions"]>[0], parseOptions: ParseOptions) {
    ...
    const externalBuiltInCommandsInfo: CommandInfo[] = [
      {
        name: "serve [entries...]",
        alias: ["server", "s"],
    ...
    const isCommand = (input: string, commandOptions: CommandOptionType) => {
    ...
    const loadCommandByName = async (commandName: CommandInfo["name"], allowToInstall = false) => {
      ...
      if (error.code === "commander.unknownOption") {
        ...
        if (name) {
          ...
          if (operand) {
            ...
            (command as WebpackCommand).options.forEach((option) => {
    ...
    const cli: WebpackCliInterface = this;
    ...
    const outputVersion = async (options: string[]) => {
      ...
      if (possibleCommandNames.length > 0) {
        ...
        for (const possibleCommandName of possibleCommandNames) {
          const foundCommand = findCommandByName(possibleCommandName) as ExtendedCommand;
          ...
          try {
            const { name, version } = this.loadJSONFile<{ name: string; version: string }>(
    ...
    const outputHelp = async (
      ...
      if (isGlobalHelp || isCommandHelp) {
        program.configureHelp({
          ...
          commandUsage: (command: WebpackCommand) => {
            let parentCmdNames = "";
          ...
          // Support multiple aliases
          subcommandTerm: (command: WebpackCommand) => {
            ...
            const args = command._args
              .map((arg: typeof Option) => humanReadableArgumentName(arg))
              .join(" ");
          ...
          visibleOptions: function visibleOptions(command: WebpackCommand): typeof Option[] {
            return command.options.filter((option: typeof Option) => {
              if (option.hidden) {
                return false;
              }
          ...
          padWidth(command: WebpackCommand, helper: Help) {
            return Math.max(
          ...
          formatHelp: (command: WebpackCommand, helper: Help) => {
            ...
            // Global options
            const globalOptionList = program.options.map((option: typeof Option) =>
              formatItem(helper.optionTerm(option), helper.optionDescription(option)),
            );
    ...
    this.program.action(async (options, program: WebpackCommand) => {
  ...
  isFunction(property: unknown): property is CallableFunction {
    return typeof property === "function";
  }

  async loadConfig(options: DevServerOptions) {
      ...
      if (interpreted) {
        ...
        try {
          ...
          if (error.failures) {
            ...
            error.failures.forEach((failure: WebpackFailure) => {
    ...
    if (options.merge) {
      const merge = await this.tryRequireThenImport<
        (firstConfig: object | object[], ...configurations: object[]) => object
      >("webpack-merge");
  ...
  async buildConfig(
    ...
    const runFunctionOnEachConfig = (options: unknown[] | unknown, fn: CallableFunction) => {
      if (Array.isArray(options)) {
    ...
    const internalBuildConfig = (item: WebpackConfiguration) => {
      ...
      if (this.webpack.cli) {
        ...
        const values: ProcessArgumentsValues = Object.keys(options).reduce(
          (accumulator: ProcessArgumentsValues, name) => {
            if (name === "argv") {
              return accumulator;
            }
        ...
        const isFileSystemCacheOptions = (
          ...
          return (
            (config as FileSystemCacheOptions).cache &&
            (config as FileSystemCacheOptions).cache.type === "filesystem"
          );
        };
  ...
  async createCompiler(
    ...
    if (compiler && (compiler as LegacyCompiler).compiler) {
      compiler = (compiler as LegacyCompiler).compiler;
    }
  ...
  async runWebpack(options: Options, isWatchCommand: boolean): Promise<void> {
    ...
    if (options.json) {
      const jsonExt = await this.tryRequireThenImport<{ stringifyStream: typeof stringifyStream }>(
    ...
    const callback = (error: WebpackError, stats: CliStats): void => {
      if (error) {
        this.logger.error(error);
        process.exit(2);
      }
      ...
      const statsOptions = isMultipleCompiler(compiler)
        ? {
      ...
      if (isMultipleCompiler(compiler) && statsForWebpack4) {
        (statsOptions as StatsOptions).colors = (
          statsOptions as { children: StatsOptions[] }
