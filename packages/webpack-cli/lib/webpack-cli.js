...
class WebpackCLI {
    ...
    async getBuiltInOptions() {
        ...
        const webpack = await this.webpack;
        ...
        const builtInFlags = [
            ...
            },
            {
                ...
                multiple: webpack.cli !== undefined,
        ...
        const coreFlags = webpack.cli
            ? Object.entries(webpack.cli.getArguments()).map(([flag, meta]) => {
    ...
    get webpack() {
        return new Promise((resolve) => {
            if (this._webpack) {
                resolve(this._webpack);

                return;
            }

            resolve(this.loadPackage(process.env.WEBPACK_PACKAGE || 'webpack'));
        }).then((result) => {
            this._webpack = result;
    ...
    async run(args, parseOptions) {
        ...
        const loadCommandByName = async (commandName, allowToInstall = false) => {
            ...
            if (isBuildCommandUsed || isWatchCommandUsed) {
                const options = await this.getBuiltInOptions();
                ...
                try {
                    loadedCommand = await this.loadPackage(pkg);
                } catch (error) {
        ...
        const outputVersion = async (options) => {
            ...
            if (possibleCommandNames.length > 0) {
                ...
                for (const possibleCommandName of possibleCommandNames) {
                    ...
                    try {
                        const { name, version } = await this.loadPackage(`${foundCommand.pkg}/package.json`);
            ...
            const pkgJSON = await this.loadPackage('../package.json');
            const webpack = await this.webpack;

            this.logger.raw(`webpack ${webpack.version}`);
            this.logger.raw(`webpack-cli ${pkgJSON.version}`);

            if (this.utils.packageExists('webpack-dev-server')) {
                // eslint-disable-next-line
                const { version } = await this.loadPackage('webpack-dev-server/package.json');
    ...
    async resolveConfig(options) {
        const loadConfig = async (configPath) => {
            ...
            try {
                options = await this.loadPackage(configPath);
            } catch (error) {
                this.logger.error(`Failed to load '${configPath}' config`);

                if (await this.isValidationError(error)) {
                    this.logger.error(error.message);
                } else {
                    this.logger.error(error);
                }
        ...
        if (options.merge) {
            const { merge } = await this.loadPackage('webpack-merge');
    ...
    async applyOptions(config, options) {
        ...
        if (webpack.cli) {
            const processArguments = async (configOptions) => {
                const builtInoptions = await this.getBuiltInOptions();
                const args = builtInoptions
                ...
                const problems = webpack.cli.processArguments(args, configOptions, values);
            ...
            config.options = Array.isArray(config.options)
                ? await Promise.all(config.options.map((options) => processArguments(options)))
                : await processArguments(config.options);
        ...
        const applyStatsColors = async (configOptions) => {
            // TODO remove after drop webpack@4
            const webpack = await this.webpack;
        ...
        config.options = Array.isArray(config.options)
            ? await Promise.all(config.options.map((options) => applyStatsColors(options)))
            : await applyStatsColors(config.options);

        return config;
    }

    async applyCLIPlugin(config, cliOptions) {
        const addCLIPlugin = async (configOptions) => {
            if (!configOptions.plugins) {
                configOptions.plugins = [];
            }

            const CLIPlugin = await this.loadPackage('./plugins/CLIPlugin');
        ...
        config.options = Array.isArray(config.options)
            ? await Promise.all(config.options.map((options) => addCLIPlugin(options)))
            : await addCLIPlugin(config.options);
    ...
    async createCompiler(options, callback) {
        ...
        try {
            compiler = webpack(
                ...
                callback
                    ? async (error, stats) => {
                          if (error && (await this.isValidationError(error))) {
                              this.logger.error(error.message);
                              process.exit(2);
                          }
    ...
    async buildCommand(options, isWatchCommand) {
        ...
        const callback = async (error, stats) => {
            ...
            if (options.json) {
                const { stringifyStream: createJsonStringifyStream } = await this.loadPackage('@discoveryjs/json-ext');
    ...
    async loadPackage(packageName) {
        ...
        try {
            ...
            const dynamicImportLoader = utils.dynamicImportLoader();
            ...
            if (
                ...
                const urlForConfig = pathToFileURL(packageName);
