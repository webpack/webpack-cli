...
const coreNegativeFlags = {
    devtool: true,
};

const coreFlagAliasMap = {
    devtool: 'd',
};
...
const coreFlags = cli
    ? Object.entries(cli.getArguments()).map(([flag, meta]) => {
          ...
          if (coreNegativeFlags.hasOwnProperty(flag)) {
              meta.negative = true;
          }

          if (coreFlagAliasMap.hasOwnProperty(flag)) {
              meta.alias = coreFlagAliasMap[flag];
          }
