...
class WebpackCLI implements IWebpackCLI {
  ...
  async run(args: Parameters<WebpackCLICommand["parseOptions"]>[0], parseOptions: ParseOptions) {
    ...
    this.program.action(async (options, program: WebpackCLICommand) => {
      ...
      const defaultCommandToRun = WebpackCLI.#commands.build.rawName;
      ...
      for (const command of Object.values(WebpackCLI.#commands)) {
        if (
          command.rawName === commandToRun ||
          (Array.isArray(command.alias)
            ? command.alias.includes(commandToRun)
            : command.alias === commandToRun)
        ) {
          isKnownCommand = true;
          break;
        }
      ...
      if (isKnownCommand) {
        ...
        if (isEntrySyntax) {
          ...
          await this.#loadCommandByName(commandToRun);
        } else {
          ...
          if (found) {
            this.logger.error(
              `Did you mean '${found.rawName}' (alias '${
