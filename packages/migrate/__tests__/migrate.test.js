...
describe("transform", () => {
    it("should not transform if no transformations defined", (done) => {
        migrate_1.transform(input, []).then((output) => {
    ...
    it("should transform only using specified transformations", (done) => {
        migrate_1.transform(input, [migrate_1.transformations.loadersTransform]).then((output) => {
            expect(output).toMatchSnapshot();
            done();
        });
    });
    it("should respect recast options", (done) => {
        migrate_1.transform(input, undefined, {
            quote: "double",
            trailingComma: true,
        }).then((output) => {
            expect(output).toMatchSnapshot();
