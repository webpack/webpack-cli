    ...
    findPluginsByName(j, ast, [pluginVariableAssignment]).forEach((node: Node): void => {
        ...
        if (pluginOptions.length) {
            ...
            if (pluginVariableAssignment) {
                ...
                ast.find(j.Program).replaceWith((p: Node): Node => j.program([].concat(pathRequire).concat((p.value as Node).body)));
        ...
        if (expressionContent) {
            ((configBody as Node).value as Node).properties.push(
                j.property('init', j.identifier('optimization'), j.objectExpression([minimizeProperty, expressionContent])),
            );
        } else {
            ((configBody as Node).value as Node).properties.push(
