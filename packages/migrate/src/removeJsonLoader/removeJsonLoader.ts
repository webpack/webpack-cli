import { safeTraverse } from '@webpack-cli/generators';
...
export default function (j: JSCodeshift, ast: Node): Node {
        ...
        switch (loadersNode.type) {
            case j.ArrayExpression.name: {
                const loaders: Node[] = loadersNode.elements.map(
                    (p: Node): Node => {
                        return safeTraverse(p, ['properties', '0', 'value', 'value']) as Node;
                    },
                );
    ...
    function removeLoaders(astNode: Node): void {
        astNode.find(j.Property, { key: { name: 'use' } }).forEach((path: Node): void => removeLoaderByName(path, 'json-loader'));

        astNode.find(j.Property, { key: { name: 'loader' } }).forEach((path: Node): void => removeLoaderByName(path, 'json-loader'));
    }
