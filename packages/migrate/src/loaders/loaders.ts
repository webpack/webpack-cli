...
export default function (j: JSCodeshift, ast: Node): Node {
                    ...
                    pairValue.elements.map(
                        (arrElement: Node): Node => {
                            ...
                            if (arrElement.type === j.Literal.name) {
                                ...
                                return j.objectExpression([createProperty(j, 'loader', arrElement.value as Node)]);
                            }
    ...
    const findLoaderWithQueryString = (p: Node): boolean => {
        return (p.value as Node).properties.reduce((predicate: boolean, prop: Node): boolean => {
            return (
                (safeTraverse(prop, ['value', 'value', 'indexOf']) && ((prop.value as Node).value as string).indexOf('?') > -1) || predicate
            );
        }, false);
    };
    ...
    const checkForLoader = (path: Node): boolean =>
        (path.value as Node).name === 'loaders' && safeTraverse(path, ['parent', 'parent', 'parent', 'node', 'key', 'name']) === 'module';
    ...
    const loadersToArrayExpression = (): Node | void =>
        ast
            ...
            .filter((path: Node): boolean => safeTraverse(path, ['parent', 'parent', 'node', 'key', 'name']) === 'rules')
