...
export default function (j: JSCodeshift, ast: Node): Node {
    ...
    CommonsChunkPlugin.forEach((path: Node): void => {
        ...
        CCPProps.forEach((p: Node): void => {
            ...
            switch (propKey) {
                case 'names':
                    (p.value as Node).elements.forEach(({ value: chunkValue }): void => {
                        if (chunkValue === 'runtime') {
                            optimizationProps['runtimeChunk'] = j.objectExpression([createProperty(j, 'name', chunkValue)]);
                        } else {
                ...
                case 'minSize':
                    ...
                    cacheGroup[chunkKey].push(j.property('init', createIdentifierOrLiteral(j, propKey), p.value as Node));
                    break;
                ...
                case 'minChunks': {
                    ...
                    if ((pathValue as Node).type === 'ArrowFunctionExpression' || (pathValue as Node).type === 'FunctionExpression') {
                        if (!Array.isArray(cacheGroup[chunkKey])) {
                            cacheGroup[chunkKey] = [];
                        }
        ...
        Object.keys(cacheGroup).forEach((chunkName: string): void => {
            ...
            commonCacheGroupsProps = commonCacheGroupsProps.filter((commonProp): boolean => !chunkPropsKeys.includes(commonProp.key.name));

            chunkProps.push(...commonCacheGroupsProps);

            if (chunkCount > 1) {
                chunkProps.push(j.property('init', createIdentifierOrLiteral(j, 'minChunks'), createIdentifierOrLiteral(j, chunkCount)));
            }
            ...
            cacheGroups.push(j.property('init', createIdentifierOrLiteral(j, chunkName), j.objectExpression([...chunkProps])));
        });
    ...
    if (cacheGroupsProps.length > 0) {
        rootProps.push(j.property('init', createIdentifierOrLiteral(j, 'cacheGroups'), j.objectExpression([...cacheGroupsProps])));
    }
