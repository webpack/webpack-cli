...
────────────────────────────────────────────────────────────────────────
  ...
  --optimization-minimize-options-css-shorten-media-queries                          Write a media feature in its range spelling where the target reads one (`(min-width:100px)` -> `(width>=100px)`), and collapse an `and` of two one-sided ranges into the interval it describes. On by default.
  ...
  --optimization-minimize-options-html-collapse-boolean-attributes                   Write a boolean attribute spelled with its own name (`disabled="disabled"`) as the bare name the spec canonicalizes it to. On by default.
  ...
  --optimization-minimize-options-html-minify-json                                   Strip the whitespace between the tokens of a `<script>` whose type is a JSON MIME type. Every literal is copied byte for byte, so no number is rounded and no escape rewritten. On by default.
  --no-optimization-minimize-options-html-minify-json                                Negative 'optimization-minimize-options-html-minify-json' option.
  --optimization-minimize-options-html-minify-srcdoc                                 Minify the document held in an `<iframe srcdoc>` attribute. Off by default: the body is a whole document of its own (its base URL is `about:srcdoc`), so minifying it is safe, but the attribute is readable from script and a consumer comparing `iframe.srcdoc` byte for byte would see it change.
  --no-optimization-minimize-options-html-minify-srcdoc                              Negative 'optimization-minimize-options-html-minify-srcdoc' option.
  --optimization-minimize-options-html-minify-styles                                 Run the CSS minimizer over an inline `<style>` element and every `style=""` attribute, with the options `optimization.minimize.css` names. On by default.
  --no-optimization-minimize-options-html-minify-styles                              Negative 'optimization-minimize-options-html-minify-styles' option.
  ...
  --optimization-minimize-options-html-normalize-list-attributes                     Normalize a list-shaped attribute value: a space-separated token list (`class`, `rel`, `part`, …), a comma-separated one (`accept`, `sizes`, …), a `srcset` and the viewport `<meta content>`. On by default. Reordering a token list is `sortTokenLists`, which is separate and off by default.
