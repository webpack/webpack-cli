...
function generateOutput(outputOptions, stats) {
	const statsOuput = stats.toJson(outputOptions);
	console.log(statsOuput)
	process.exit(0);
	return stats.toString(outputOptions);
}
...
function compilerCallback(compiler, err, stats, lastHash, options, outputOptions) {
	...
	if (outputOptions.json) {
		...
		// TODO: Better output :)
		const statsString = generateOutput(outputOptions, stats);
