...
class webpackCLI extends ErrorHelper {
	...
	runOptionGroups() {
		// TODO: each group has newObj props, only loop through each group
		const res = this.groups.map(Group => Group.run()).filter(e => e);
		let newObj = {
			outputOptions: {},
			options: {},
			processingErrors: []
		...
		for(let k of res) {
			...
			if(k.outputOptions) {
				newObj.outputOptions = Object.assign(newObj.outputOptions, k.outputOptions);
			}
			if(k.options) {
				newObj.options = Object.assign(newObj.options, k.options);
			}
		}
		return newObj;
	}
	...
	async run(args, yargsOptions) {
		...
		const webpack = await Compiler(res);
		return webpack;
	}
