...
class webpackCLI extends GroupHelper {
	...
	runOptionGroups() {
		...
		let tmpOpts = {};
		Object.keys(groupResult.options).forEach( opt => {
		/* 	console.log(opt, groupResult.options[opt]) */
			const optRes = groupResult.options[opt];
			if(Array.isArray(optRes) && optRes.length > 0) {
				tmpOpts[opt] = groupResult.options[opt];
			}
			else if(optRes && !Array.isArray(optRes)) {
				tmpOpts[opt] = groupResult.options[opt]
			}
		})
		groupResult.options = tmpOpts;
		return groupResult
	}
