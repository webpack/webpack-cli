...
class WebpackCLI extends GroupHelper {
        ...
        for (const [key, value] of this.groupMap.entries()) {
            switch (key) {
                ...
                case groups.HELP_GROUP: {
                    ...
                    this.groups.push(this.helpGroup);
                    break;
                }
    ...
    async runOptionGroups() {
        ...
        // const resolvedResults = await Promise.all(this.groups.map(Group => Group.run()).filter(e => e));
    ...
    async getCompiler(args, yargsOptions) {
        await this.processArgs(args, yargsOptions);
