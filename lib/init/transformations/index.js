...
module.exports = function runTransform(webpackProperties, action) {
	...
	Object.keys(webpackProperties).forEach(scaffoldPiece => {
		...
		const ast = j((action ? webpackProperties.configFile : "module.exports = {}"));
		const transformAction = action || "init";
		...
		return pEachSeries(transformations, f => {
			...
			if (!f[1]) {
				...
				return f[0](j, ast, transformAction, f[1]);
			}
		})
			.then(() => {
				...
				const outputPath = action ? webpackProperties.configPath : path.join(process.cwd(), configurationName);
