...
module.exports = function runTransform(webpackProperties, options, action) {
	// Todo: Generalize, extract into multiple functions and reuse
	if (action === "add" || action === "remove") {
		let transformAction = webpackProperties.shift();
		let item = webpackProperties.pop();

		const transformations = Object.keys(transformsObject)
			.map(k => {
				const stringVal = k.substr(0, k.indexOf("Transform"));
				if (transformAction === stringVal) {
					return [transformsObject[k]];
				}
			})
			.filter(e => e);
		const ast = j(options.config);
		return pEachSeries(transformations, f => f[0](j, ast, item, action))
			.then(() => {
				const source = ast.toSource({
					quote: "single"
				});
				runPrettier(options.configPath, source);
