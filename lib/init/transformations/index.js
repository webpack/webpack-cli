	...
	if (action === "add" || action === "remove") {
		const transformations = Object.keys(transformsObject)
			.map(k => {
				...
				// Todo: change
				if (stringVal === "something") {
					return [transformsObject[k]];
				}
			})
			.filter(e => e);
		const ast = j(webpackProperties.configFile);
		// TODO: add args to transform
		return pEachSeries(transformations, f => f[0](j, ast))
			.then(() => {
				const source = ast.toSource({
					quote: "single"
				});
				runPrettier(webpackProperties.configPath, source);
				//TODO: update in literal
