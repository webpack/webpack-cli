...
module.exports = function runTransform(webpackProperties, action) {
	...
	const webpackConfig = action !== "init"
	...
	Object.keys(webpackConfig).forEach(scaffoldPiece => {
		...
		const ast = j(
			action !== "init" ? webpackProperties.configFile : "module.exports = {}"
		);

		return pEachSeries(transformations, f => {
			if (!f[1]) {
				return f[0](j, ast, action);
			} else {
				return f[0](j, ast, f[1], action);
			}
		})
			.then(_ => {
