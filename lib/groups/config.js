...
class ConfigGroup extends GroupHelper {
        ...
        if (moduleDescriptor) {
            if (typeof moduleDescriptor === 'string') {
                ...
                for (let i = 0; i < moduleDescriptor.length; i++) {
                    try {
                        this.registerCompiler(moduleDescriptor[i]);
                        break;
                    } catch (e) {
                        // do nothing
                    }
    ...
    resolveConfigFiles() {
        ...
        if (!this.configFiles) {
            this.configFiles = this.defaultConfigFiles.filter(defaultFile => existsSync(defaultFile)).map(this.mapConfigArg.bind(this));
        }
        // grab array config etc
        if (this.configFiles.length < 1) {
            const defaultConfig = this.defaultConfigFiles
