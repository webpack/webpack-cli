...
class ConfigGroup extends GroupHelper {
	...
	getDefaultConfigFiles() {
		...
		let DEFAULT_FILES = ["webpack.config", "webpack.dev.config", "webpackfile"];
		if(mode === 'prod') {
			DEFAULT_FILES.unshift("webpack.prod.config");
		}
	...
	resolveConfigFiles() {
		...
		if (config) {
			this.configFiles = existsSync(config) ? this.mapConfigArg(config) : null;
		}
		if(!this.configFiles) {
		...
		if(this.configFiles.length < 1) {
			this.configFiles = [this.defaultConfigFiles.find(defaultFile =>
				existsSync(defaultFile)
			)].filter(e => e).map(this.mapConfigArg.bind(this));
		}
