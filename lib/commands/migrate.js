...
module.exports = class MigrateCommand {
	constructor() {
		this.name = "migrate";
	}

	// there was an options here, but didnt came from lib/index. I'm kinda confused.
	run({ args, popArgs }) {
		...
		const currentConfigPath = inputConfigPath;
		const outputConfigPath = inputConfigPath;
				...
				return inquirer
					...
					.then((answer) => {
						...
						if (answer["confirmValidation"]) {
							const webpackOptionsValidationErrors = validate(require(outputConfigPath));
