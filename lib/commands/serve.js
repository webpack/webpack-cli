 ...
 *
 */
module.exports = class ServeCommand {
	constructor() {
		this.name = "serve";
	}
	...
	*/
	getRootPathModule(dep) {
		return path.resolve(process.cwd(), dep);
	}
	...
	*/
	...
	run() {
		const packageJSONPath = this.getRootPathModule("package.json");
		if (!packageJSONPath) {
			console.log(
				"\n",
		...
		*/
		...
		if (hasDevServerDep.length) {
			let WDSPath = this.getRootPathModule(
				...
				.then(answer => {
					if (answer["confirmDevserver"]) {
						return inquirer
							...
							.then(depTypeAns => {
								const packager = this.getRootPathModule("package-lock.json")
									? "npm"
									: "yarn";
								let spawnAction;
								if (depTypeAns["confirmDepType"] === "devDependency") {
									if (packager === "yarn") {
										spawnAction = _ => this.spawnYarnWithArg("--dev");
									} else {
										spawnAction = _ => this.spawnNPMWithArg("--save-dev");
									}
								}
								if (depTypeAns["confirmDepType"] === "dependency") {
									if (packager === "yarn") {
										spawnAction = _ => this.spawnYarnWithArg(" ");
									} else {
										spawnAction = _ => this.spawnNPMWithArg("--save");
									}
								}
								if (depTypeAns["confirmDepType"] === "optionalDependency") {
									if (packager === "yarn") {
										spawnAction = _ => this.spawnYarnWithArg("--optional");
									} else {
										spawnAction = _ => this.spawnNPMWithArg("--save-optional");
									}
								}
