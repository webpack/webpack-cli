...
function generateOutputForSingleCompilation(statsObj, statsErrors, processingMessageBuffer) {
    const { assets, entrypoints, time, builtAt, warnings, outputPath, hash } = statsObj;

    console.log(statsObj);
    const visibleEmojies = showEmojiConditionally() ? ['✅', '🌏', '⚒️ ', '⏱ ', '📂', '#️⃣'] : new Array(6).fill('');
    ...
    const assetsTble = new Table({
        head: ['Entrypoint', 'Bundle name', 'Compared for emit', 'Bundle size'],
        colWidths: [20, 20, 45, 15],
    ...
    Object.keys(entrypoints).forEach(entry => {
        ...
        entrypoint.assets.forEach(assetName => {
            ...
            assetsTble.push([entrypoint.name, asset.name, hasBeenCompiled, kbSize]);
