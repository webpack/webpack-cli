    ...
    if (outputOptions.json) {
        ...
        if (stats.compilation && stats.compilation.errors.length !== 0) {
            ...
            if (errors[0].name === 'EntryModuleNotFoundError') {
                logger.error('Insufficient number of arguments or no entry found.');
                logger.error("Alternatively, run 'webpack(-cli) --help' for usage info.");
            }
        }
...
module.exports = function webpackInstance(opts) {
    ...
    if (outputOptions.help) {
        logger.error(outputOptions.help);
        return;
    }
    ...
    try {
        ...
        if (err.name === 'WebpackOptionsValidationError') {
            if (outputOptions.color) logger.error(`\u001b[1m\u001b[31m${err.message}\u001b[39m\u001b[22m`);
    ...
    console.log('\n');
    logger.info(`webpack ${webpack.version}`);
    ...
    if (outputOptions.watch) {
        ...
        if (outputOptions.infoVerbosity !== 'none') logger.info('watching the files...');
