...
function compilerCallback(compiler, err, stats, lastHash, options, outputOptions) {
    let statsErrors = [];
    // TODO handle this in default reporter
    ...
    // }
    // TODO handle this in the default reporter
...
module.exports = async function webpackInstance(opts, shouldUseMem) {
    ...
    const ReporterPlugin = require('test-webpack-reporter-plugin');
    const reporterPluginExists = options.plugins ? options.plugins.find(p => p instanceof ReporterPlugin) : false;
    let reporterPluginOptions;
    if (reporterPluginExists) {
        options.plugins = options.plugins.filter(e => e !== reporterPluginExists);
        reporterPluginOptions = reporterPluginExists.options;
    }
    new ReporterPlugin(reporterPluginOptions).apply(compiler, outputOptions);
    ...
    compiler.hooks.beforeRun.tap('webpackProgress', compilation => {
        if(outputOptions.progress) { // TODO prevent the ProgressPlugin from priting
            let tmp_msg;
            const defaultProgressPluginHandler = (percent, msg, addInfo) => {
                ...
                if (msg && tmp_msg != msg) {
                    process.cliLogger.info(percent + '%kjjlkjklklj ' + msg);
                }
                tmp_msg = msg;
            };
            if (!progressPluginExists) {
    ...
    // TODO handle infoVerbosity in default plugin
