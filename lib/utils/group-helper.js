...
const parseAndValidateFilename = pathToResolve => {
    ...
    const isPathNotHavingIndexFile = ['index', 'index.js'].every(file => pathToArray.slice(-1).pop() !== file);
...
class GroupHelper {
    ...
    resolveFilePath(filename = '', defaultValue) {
        ...
        let predefinedConfigPath, configPathExists;
        if (filename) {
            if (!defaultValue) {
                [filename, defaultValue] = parseAndValidateFilename(filename);
                predefinedConfigPath = filename ? resolve(process.cwd(), filename, defaultValue) : null;
            } else {
                predefinedConfigPath = filename ? resolve(process.cwd(), filename) : null;
            }
            let configPath;
            configPathExists = predefinedConfigPath ? existsSync(predefinedConfigPath) : false;
            return configPathExists ? predefinedConfigPath : configPath;
        }

        defaultValue = defaultValue || 'index.js';
        ...
        let configPath;
        predefinedConfigPath = filename ? resolve(process.cwd(), filename) : undefined;
        configPathExists = predefinedConfigPath ? existsSync(predefinedConfigPath) : undefined;
