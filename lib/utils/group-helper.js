    ...
    resolveFilePath(filename = null, defaultValue) {
        ...
        if (filename) {
            if (!defaultValue) {
                ...
                defaultValue = fileNameArray[fileNameArray.length - 1]
                const isLastNameAFile = existsSync(
                    resolve(
                        join(
                            fileNameArray.slice(0, fileNameArray.length - 1).join('/'),
                            defaultValue.includes('.js') ? defaultValue : defaultValue + ".js"
                        )
                    )
                );
                defaultValue = isLastNameAFile ? defaultValue : 'index.js';
                defaultValue = defaultValue.includes('.js') ? defaultValue : defaultValue + ".js"
                filename = (defaultValue == 'index.js' && (fileNameArray[fileNameArray.length - 1] != 'index' && fileNameArray[fileNameArray.length - 1] != 'index.js')) ? fileNameArray.join('/') : fileNameArray.slice(0, fileNameArray.length - 1).join('/')
            }
            let configPath;
