...
/**
 * Adds a require statement for a given package
 *
 * @param {any} j — jscodeshift API
 * @param {Node} node - Node to start search from
 * @param {String} constName - Name of require
...
function addRequire(j, node, constName, packagePath) {
	const pathRequire = getRequire(j, constName, packagePath);
	node.find(j.Program).replaceWith(p => j.program([].concat(pathRequire).concat(p.value.body)));
}

/**
 *
 * Creates a new plugin with the specified options
 *
 * @param {any} j — jscodeshift API
 * @param {Node} node - Node to start search from
 * @param {String} pluginName - ex. `webpack.LoaderOptionsPlugin`
...
function createPluginByName(j, node, pluginName, options) {
	node.find(j.Identifier, { name: "plugins" })
		.filter(path => safeTraverse(path, ["parent", "value"]))
		.forEach(path => {
			createOrUpdatePluginByName(j, path.parent.value, pluginName, options);
		});
}

/**
 *
 * Gets the value associated with a plugin option
 *
 * @param {any} j — jscodeshift API
 * @param {Node} node - Node to start search from
 * @param {String} pluginName - Name of plugin on which to look for options
 * @param {String} name - Key of an option to grab from the plugin
 * @returns {any} - Plugin option value
 */

function getPluginOptionValue(j, node, pluginName, name) {
	let value;
	const parentPath = ["parent", "parent", "parent", "value", "callee"];
	const valuePath = ["parent", "value", "value", "value"];
	node.find(j.Identifier, { name })
		.filter(path => safeTraverse(path, parentPath))
		.filter(path => j.match(parentPath.reduce((a, b) => a[b], path), { name: pluginName }))
...
/**
 *
 * Gets the value associated with a loader option
 *
 * @param {any} j — jscodeshift API
 * @param {Node} node - Node to start search from
 * @param {String} pluginName - Name of plugin on which to look for options
 * @param {String} name - Key of an option to grab from the plugin
 * @returns {any} - Loader option value
 */

function getLoaderOptionValue(j, node, loaderName, name) {
	let value = null;
	const parentPath = ["parent", "parent", "parent", "value", "callee", "object"];
	const valuePath = ["parent", "value", "value"];
	node.find(j.Identifier, { name })
		.filter(path => safeTraverse(path, parentPath))
		.filter(path => j.match(parentPath.reduce((a, b) => a[b], path), { name: loaderName }))
		.filter(path => safeTraverse(path, valuePath))
		.forEach(path => {
			value = valuePath.reduce((a, b) => a[b], path);
		});
...
function removePluginByName(j, node, pluginName) {
	let rootPath;
...
/**
 *
 * Removes a require statement for a given package
 *
 * @param {any} j — jscodeshift API
 * @param {Node} node - Node to start search from
 * @param {String} name - dependancy to remove
 * @returns {Boolean} - true if the require statement was found
 */

function removeRequire(j, node, name) {
	let found = false;
	node.find(j.Identifier, { name })
...
function pathToArray(path, action) {
...
/**
 * Finds a loader call expression, e.g. use: ExtractTextPlugin.extract({ ... })
 *
 * @param {any} j — jscodeshift API
 * @param {Node} node - Node to start search from
 * @param {String} name - loader call to remove
 * @returns {Node | Void} - path to the targeted loader call expression if found
 */

function findLoaderFunction(j, node, name) {
	let loaderPath;
	node.find(j.Identifier, { name })
		.filter(path => safeTraverse(path, ["parent", "value"]))
		.filter(path => isType(path.parent.value, "MemberExpression"))
		.forEach(path => {
			loaderPath = safeTraverse(path, ["parent", "parent"]);
...
/**
 * Creates a new conditional loader array node based on NODE_ENV
 *
 * @param {any} j — jscodeshift API
 * @param {Array} prodLoaders - Nodes for production loaders
 * @param {Array} devLoaders - Nods for development loaders
 * @returns {Node} - New loader node
 */

function addConditionalLoader(j, prodLoaders, devLoaders) {
