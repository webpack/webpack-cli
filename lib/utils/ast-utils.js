...
/**
 * Find plugins property in config and
 * remove if it has no elements.
 * @param  {any} j - jscodeshift API
 * @param  {Node} rootNode - node to start search from (config object NODE)
 */

function findPluginsArrayAndRemoveIfEmpty(j, rootNode) {
	rootNode.find(j.Identifier, { name: "plugins" }).forEach((node) => {
