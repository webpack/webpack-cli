...
function addProperty(j, p, key, value) {
	...
	if (Array.isArray(value)) {
		valForNode = createArrayWithChildren(j, value);
	} else if (typeof value === "object") {
		// object -> loop through it
		valForNode = pushObjectKeys(j, p, value);
	} else {
		valForNode = createIdentifierOrLiteral(j, value);
	}
	const method = j.property("init", j.identifier(key), valForNode);
	if (p.properties) {
		p.properties.push(method);
	} else {
		p.value.properties.push(method);
	}
	return p;
}
module.exports = {
