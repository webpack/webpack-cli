...
class CompilerOutput {
    generateFancyOutput(statsObj, statsErrors, processingMessageBuffer) {
        ...
        const assetsTble = new Table({
            head: ['Entrypoint', 'Bundle name', 'Compared for emit', 'Modules', 'Bundle size'],
            colWidths: [20, 20, 45, 45, 15],
        ...
        Object.keys(entrypoints).forEach(entry => {
            ...
            entrypoint.assets.forEach(assetName => {
                ...
                assetsTble.push([entrypoint.name, asset.name, hasBeenCompiled, chunksOutput, kbSize]);
