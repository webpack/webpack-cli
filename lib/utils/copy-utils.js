 ...
 */
const generatorCopy = (generator, templateDir) => /** @param {string} filePath */ (filePath) => {
 ...
 * the template files.
 */
const generatorCopyTpl = (generator, templateDir, templateData) => /** @param {string} filePath */ (filePath) => {
	const sourceParts = templateDir.split(path.delimiter);
	sourceParts.push.apply(sourceParts, filePath.split('/'));
	const targetParts = path.dirname(filePath).split('/');
