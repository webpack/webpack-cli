			...
			p.value.properties.filter( node => node.key.name === 'output').forEach( (prop) => {
				...
				Object.keys(webpackProperties.output).forEach( (webpackProp) => {
					if(webpackOutputTypes.includes(webpackProp)) {
						if(webpackProperties.output[webpackProp] && webpackProperties.output[webpackProp].__paths) {
							let RegExpDec = webpackProperties.output[webpackProp].__paths[0].value.program.body[0];
							if(RegExpDec.expression.callee) {
								prop.value.properties.push(
									j.property('init', j.identifier(webpackProp), RegExpDec.expression)
								);
							} else {
								prop.value.properties.push(
									j.property('init', j.identifier(webpackProp), j.literal(RegExpDec.expression.value))
								);
							}
						} else {
							prop.value.properties.push(
