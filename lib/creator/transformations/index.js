...
const topScopeTransform = require('./topScope/index');
...
module.exports = function runTransform(yeomanConfig) {
	...
	if (yeomanConfig.webpackOptions) {
		...
		return pEachSeries(transformations, f => f(j, ast, yeomanConfig.webpackOptions))
			.then(() => {
				if(yeomanConfig.topScope) {
					topScopeTransform(j, ast, yeomanConfig.topScope);
				}
				const recastOptions = {
					quote: 'single'
				};
				...
				const runPrettier = () => {
					...
					const spawnPrettier = () => {
						console.log('\n');
						return spawn('prettier', ['--single-quote', '--trailing-comma es5', '--write', outputPath], { stdio: 'inherit', customFds: [0, 1, 2] });
					};
