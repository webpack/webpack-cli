...
module.exports = function(j, ast, yeomanConfig) {
	...
	function createModuleProperties(p) {
		...
		let moduleNode = p.value.properties.filter(n => n.key.name === 'module');
		moduleNode.forEach( (prop) => {
					...
					if(webpackProperties.module[webpackProp].__paths) {
						...
						prop.value.properties.push(
							j.property(
								'init',
								j.identifier(webpackProp),
								j.identifier(webpackProperties.module[webpackProp])
							)
						);
					}
				}
	...
	if(!webpackProperties['module']) {
		...
		.filter(p => createModuleProperties(p));
	}
};
