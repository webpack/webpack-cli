	...
	function createExternalProperty(p) {
		if(p.parent.value.type === 'AssignmentExpression') {
			if(webpackProperties.externals instanceof RegExp) {
				return p.value.properties.push(utils.createProperty(j, 'externals', webpackProperties.externals));
			}
				...
				return p.value.properties.push(utils.createProperty(j, 'externals', RegExpDec));
			} else {
