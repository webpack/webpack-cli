...
const isFlagPresent = flag => process.argv.find(arg => [flag, `--${flag}`].includes(arg));
const normalizeFlags = command => process.argv.slice(2).filter(arg => arg.indexOf('--') < 0 && arg !== command.name && arg !== command.alias);
...
async function runCLI(cli, commandIsUsed) {
    let args;
    const helpFlagExists = isFlagPresent('help');
    const versionFlagExists = isFlagPresent('version');
