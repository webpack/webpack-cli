...
const validFlags = [...core.map(obj => obj.name), ...commands.map(obj => obj.name)];
...
const checkValidFlag = () => {
    let flagIsValid = true;
    process.argv.slice(2).forEach(flag => {
        const isDashed = flag.substr(0, 2) === '--';
        if (!validFlags.includes(isDashed ? flag.substr(2) : flag)) {
            flagIsValid = false;
        }
    });
    return flagIsValid;
