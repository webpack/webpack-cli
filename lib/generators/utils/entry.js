...
module.exports = (self, answer) => {
	...
	if (answer["entryType"] === true) {
		...
		let defaultName = "./src/index.js";
		result = self
			.prompt([
				Input(
					"singularEntry",
					"Which module will be the first to enter the application? [example: './src/index', default: './src/index']"
				)
			])
			.then(singularAnswer => {
				if (singularAnswer["singularEntry"].length) {
					return singularAnswer;
				} else {
					singularAnswer["singularEntry"] = defaultName;
					return singularAnswer;
				}
			})
