// @flow
import type { Ijscodeshit, IPath, IProperty, ILiteral, IIdentifier, IVariableDeclarator, IObjectExpression, IArrayExpression } from "../types"; // eslint-disable-line node/no-unsupported-features
...
* @returns Path
 * */
function findPluginsByName(j: Ijscodeshit, node: IPath<*>, pluginNamesArray: string[]): IPath<*> {
	return node
 ...
 * @returns Path
 * */
function findPluginsRootNodes(j: Ijscodeshit, node: IPath<*>): IPath<IProperty<*>> {
 ...
 * @returns Node
 * */
function createProperty(j: Ijscodeshit, key: string | number, value: any): IProperty<*> {
	return j.property(
		"init",
...
function createLiteral(j: Ijscodeshit, val: string | boolean | number): IPath<ILiteral> {
...
function createIdentifierOrLiteral(j: Ijscodeshit, val: any): any { // IPath<IIdentifier> | IPath<ILiteral> doesn't work, find another way
	let literalVal = val;
	// We'll need String to native type conversions
 ...
 * @returns void
 * */
function createOrUpdatePluginByName(j: Ijscodeshit, rootNodePath: IPath<*>, pluginName: string, options: any) {
...
function findVariableToPlugin(j: Ijscodeshit, rootNode: IPath<*>, pluginPackageName: string): ?string {
	const moduleVarNames: IVariableDeclarator[] = rootNode.find(j.VariableDeclarator)
...
function getRequire(j: Ijscodeshit, constName: string, packagePath: string): IPath<IVariableDeclarator> {
...
function checkIfExistsAndAddValue(j: Ijscodeshit, node: IPath<IObjectExpression>, key: string, value: string): void {
...
*/
function createEmptyArrayProperty(j: Ijscodeshit, key: string): IProperty<IArrayExpression> {
...
function createArrayWithChildren(j: Ijscodeshit, key: string, subProps: { [string]: any }, shouldDropKeys: boolean) {
...
function loopThroughObjects(j: Ijscodeshit, p: IObjectExpression, obj: Object): void {
	Object.keys(obj).forEach((prop) => {
		if (prop.indexOf("inject") >= 0) {
			// TODO to insert the type of node, identifier or literal
			const propertyExpression: IPath<*> = createIdentifierOrLiteral(j, obj[prop]);
			return p.properties.push(propertyExpression);
		}
		// eslint-disable-next-line no-irregular-whitespace
		if (typeof(obj[prop]) !== "object" /*|| obj[prop] instanceof RegExp*/) {
			p.properties.push(createObjectWithSuppliedProperty(
				j, prop, createIdentifierOrLiteral(j, obj[prop])
			));
			...
			let propertyOfBlock = createObjectWithSuppliedProperty(j, prop, objectBlock);
...
function createObjectWithSuppliedProperty(j: Ijscodeshit, key: string, prop: any): IProperty<*> {
...
function pushCreateProperty(j: Ijscodeshit, p: IPath<IObjectExpression>, key: string, val: any): number {
...
function pushObjectKeys(j: Ijscodeshit, p: IPath<*>, webpackProperties: Object, name: string): any {
	p.value.properties.filter(n =>
		(safeTraverse(n, ["key", "name"]) === name)
	).forEach((prop) => {
		Object.keys(webpackProperties).forEach((webpackProp) => {
			if (webpackProp.indexOf("inject") >= 0) {
			...
			} else if (typeof(webpackProperties[webpackProp]) !== "object" || webpackProperties[webpackProp].__paths || webpackProperties[webpackProp] instanceof RegExp) {
				...
				return pushObjectKeys(
					j, prop, webpackProperties[webpackProp], webpackProp
				);
			}
		});
	});
...
function isAssignment(j: Ijscodeshit, p: IPath<*>, cb: () => void, identifier: string, property: Object): any {
