...
function createArrayWithChildren(j, key, subProps, shouldDropKeys) {
	...
	if(shouldDropKeys) {
		subProps.forEach( (subProperty) => {
			arr.value.elements.push(createIdentifierOrLiteral(j, subProperty));
		});
	} else {
...
/*
* @function createPropertyWithSuppliedProperty
*
* Creates an property with an supplied property as parameter
* @param j — jscodeshift API
* @param { String } key - object name
...
function createPropertyWithSuppliedProperty(j, key, prop) {
...
function pushCreateProperty(j, p, key, val) {
	...
	return p.value.properties.push(
		createPropertyWithSuppliedProperty(j, key, property)
	);
}

/*
* @function createSingularProperty
*
* Creates an assignment with given properties from yeoman
* @param j — jscodeshift API
* @param { Node } p - Node to push against
* @param { String } name - key used as identifier
* @param { String } webpackProperties - property value to add with key
* @returns - { Node } - Returns node the pushed property
*/

function createSingularProperty(j, p, name, webpackProperties) {
	return pushCreateProperty(j, p, name, webpackProperties[name]);
}
...
module.exports = {
	...
	createPropertyWithSuppliedProperty,
	createExternalRegExp,
	pushCreateProperty,
	createSingularProperty
};
