...
module.exports = function(j, ast) {
	/**
	* Creates an Array expression out of loaders string for syntaxes like
	*
	* {
	*   loader: 'style!css`
	* }
	...
	*   }]
	* }
	* @param   {NodeObject} path  Must be an ObjectExpression
	* @returns {NodeObject} The created array expression
	*/
	const createArrayExpressionFromArray = function(path) {
		// Find paths with `loaders` keys in the given Object
		const paths = path.value.properties.filter(prop =>
		...
		paths.forEach(pair => {
			...
			if (pair.value.type === j.ArrayExpression.name) {
				...
				pair.value = j.arrayExpression(
					pair.value.elements.map(arrElement => {
				...
				pair.value = j.arrayExpression(
					pair.value.value.split("!").map(loader => {
						return j.objectExpression([
	...
	* @param   {object} p tbd
	* @returns {object} A new object that contains the pre/post loaders in the enforce key of the loader
	*/
	...
	* @returns {*}  All the pre and post loaders
	*/
	...
	* @returns {Array} An array of object of all the rules
	*/
	...
	* @returns {Array} An array of objects of all the rules
	*/
	...
	*   }
	* }
	* @returns {Object}  An object of the generated loader settings
	*/
	...
	*   options: { ... }
	* }
	* @returns {Object} An object with the replaced query into options
	*/
	...
	* @returns {object} The new object that has all the `-loader` sufixes
	*/
