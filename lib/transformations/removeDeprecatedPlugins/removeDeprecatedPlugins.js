// @flow
import type { Ijscodeshit, IPath } from "../../types"; // eslint-disable-line node/no-unsupported-features

const codeFrame = require("babel-code-frame");
const chalk = require("chalk");
const utils = require("../utils");

const example: string = `plugins: [
...
module.exports = function(j: Ijscodeshit, ast: IPath<*>, source: string) {
	...
	return utils.findPluginsByName(j, ast, deprecatedPlugingsList)
		.forEach((path: IPath<*>) => {
			...
			const arrayPath: ?IPath<*> = utils.safeTraverse(path, ["parent","value"]);
