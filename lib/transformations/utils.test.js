...
`);
	...
	describe('findWebpackRoot', () => {
		it('should find the webpack root object', () => {
			const ast = j('module.exports={}');
			expect(ast.find(j.ObjectExpression)
			.filter(utils.findWebpackRoot).size()).toEqual(1);
		});
		it('should find only webpack root object', () => {
			const ast = j('module.exports={entry: {}}');
			expect(ast.find(j.ObjectExpression)
			.filter(utils.findWebpackRoot).size()).toEqual(1);
		});
		it('should find only webpack root object 2', () => {
			const ast = j('entry: {}');
			expect(ast.find(j.ObjectExpression)
			.filter(utils.findWebpackRoot).size()).toEqual(0);
		});
	});
});
