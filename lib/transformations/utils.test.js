...
`);
	...
	describe('createObjectWithSuppliedProperty', () => {
		it('should create an object with a property supplied by us', () => {
			const ast = j('{}');
			const prop = utils.createObjectWithSuppliedProperty(j, 'its-lit', j.objectExpression([]));
	...
	describe('pushCreateProperty', () => {
		it('should create an object or property and push the value to a node', () => {
			...
			ast.find(j.Identifier).filter(n => n.value.name === 'pushMe').forEach(node => {
				const heavyNodeNoSafeTraverse = node.parentPath.value;
				utils.pushCreateProperty(j, heavyNodeNoSafeTraverse, 'just', 'pushed');
			});
			expect(ast.toSource()).toMatchSnapshot();
		});
	});
	describe('createSingularProperty', () => {
		it('should create an single property with no nested properties', () => {
			const ast = j('module.exports = {}');
			const myObj = 'Heyho';
			const myKey = 'context';

			ast.find(j.ObjectExpression)
			.filter(n => n.parent.value.type === 'AssignmentExpression')
			.forEach(node => {
				utils.createSingularProperty(j, node, myKey, myObj);
			});
			expect(ast.toSource()).toMatchSnapshot();
		});
	});
	describe('isAssignment', () => {
		it('should invoke a callback if parent type is AssignmentExpression', () => {
			...
			ast.find(j.ObjectExpression)
			.filter(n => utils.isAssignment(j, n, utils.createSingularProperty, myKey, myObj));
			expect(ast.toSource()).toMatchSnapshot();
		});
		it('should allow custom transform functions instead of singularProperty', () => {
			...
			function createPluginsProperty(p) {
				...
				const pluginArray = utils.createArrayWithChildren(j, 'plugins', webpackProperties);
