...
export default function transformer(file, api) {
	...
	const getRootIndex = p => {
		return p.node.value.properties.reduce((rootIndex, prop, index) => prop.key.name === 'root' ? index: rootIndex, -1);
	};
    ...
    .find(j.Property)
    .filter(p => p.node.key.name === 'resolve'  && p.node.value.properties.filter(prop => prop.key.name ==='root').length === 1)
