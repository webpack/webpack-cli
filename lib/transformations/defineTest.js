 ...
 */
function runSingleTansform(dirName, transformName, testFilePrefix, initOptions) {
	if (!testFilePrefix) {
	...
	if (initOptions) {
		if (initOptions.topScope) {
			return transform(jscodeshift, ast, initOptions).toSource({ quote: 'single' });
		} else {
			return transform(jscodeshift, ast, {
				webpackOptions: initOptions
 ...
 */
function defineTest(dirName, transformName, testFilePrefix, type) {
	const testName = testFilePrefix
		? `transforms correctly using "${testFilePrefix}" data`
		: 'transforms correctly';
	describe(transformName, () => {
		it(testName, () => {
			const output = runSingleTansform(dirName, transformName, testFilePrefix, type);
